...
async fn start_transient_service(conn: &Connection, options: &LaunchOptions<'_>) -> Result<(), ()> {
	...
	let (pre_entries, main_entry, post_entries) = tokio::task::spawn_blocking({
		...
		let main_command = (options.program.to_string(), options.args.to_vec());
		move || {
			(
				build_exec_entries(&pre_commands),
				build_exec_entry(main_command.0, main_command.1),
...
/// Returns (absolute_path, argv, ignore_errors=false).
fn build_exec_entry(program: String, args: Vec<String>) -> (String, Vec<String>, bool) {
	let argv: Vec<String> = std::iter::once(program.to_string())
		.chain(args.iter().cloned())
		.collect();
	(program.to_string(), argv, false)
}
