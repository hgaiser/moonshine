...
a=control:streamid=0")
	...
	pub(super) async fn run(
		...
		let video_task = tokio::task::spawn_blocking({
			let config = self.config.clone();
			let video_stream_context = self.video_stream_context.clone();
			move || {
				tokio::runtime::Handle::current().block_on(run_video_stream(
					config,
		...
		let control_task = tokio::task::spawn_blocking({
			let config = self.config.clone();
			move || {
