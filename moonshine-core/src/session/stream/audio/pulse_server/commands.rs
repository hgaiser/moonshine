...
pub(super) fn handle_command(
	...
	match cmd {
		...
		pulse::Command::GetCardInfo(_) => {
			pulse::write_error(&mut ClientWriter(&mut client.outgoing), seq, &pulse::PulseError::NoEntity)?;
			Ok(())
		},
		...
		pulse::Command::CreatePlaybackStream(params) => {
			...
			if !is_supported_format(sample_spec.format) {
				...
				pulse::write_error(&mut ClientWriter(&mut client.outgoing), seq, &pulse::PulseError::NotSupported)?;
				return Ok(());
			}
...
pulse::write_command_message(
		...
		pulse::Command::Extension(_) => {
			pulse::write_error(&mut ClientWriter(&mut client.outgoing), seq, &pulse::PulseError::NoExtension)?;
			Ok(())
		},
		...
		_ => {
			...
			pulse::write_error(&mut ClientWriter(&mut client.outgoing), seq, &pulse::PulseError::NotImplemented)?;
			Ok(())
		},
	}
}
