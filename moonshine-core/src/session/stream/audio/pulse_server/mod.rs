...
/// Cap on per-client pending outgoing data. Control messages are small, so a
/// buffer exceeding this means the client has stopped reading for a long time.
/// Rather than let the buffer grow unboundedly, the stuck client is dropped
/// (the session itself must survive a stuck client).
const MAX_OUTGOING_BUFFER: usize = 256 * 1024;
...
impl PulseServer {
		...
		let result = (|| -> Result<(), Error> {
			...
			'read: loop {
				...
				loop {
					...
					let desc =
						pulse::read_descriptor(&mut Cursor::new(&client.incoming[..pulse::DESCRIPTOR_SIZE]))?;
	...
	fn flush_client(&mut self, client_token: mio::Token) {
		...
		if remove
			&& let Some(mut client) = self.clients.remove(&client_token)
		{
			tracing::debug!("PulseAudio client disconnected (id={})", client.id);
			let _ = self.poll.registry().deregister(&mut client.socket);
		}
