...
pub(crate) struct SwapchainFactoryData;
...
impl Dispatch<GamescopeSwapchainFactoryV2, SwapchainFactoryData> for MoonshineCompositor {
	fn request(
		...
		use crate::session::compositor::protocols::gamescope_swapchain_factory_v2::Request;
		match request {
...
impl Dispatch<MoonshineSwapchainFactoryV2, SwapchainFactoryData> for MoonshineCompositor {
	fn request(
		...
		use crate::session::compositor::protocols::moonshine_swapchain_factory_v2::Request;
		match request {
			Request::CreateSwapchain { surface, callback } => {
...
pub(crate) fn register_moonshine_globals(display: &DisplayHandle) {
